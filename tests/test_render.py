...
class TestEncodeSablonu:
    def test_sablon_degerleri_spec_ile_birebir(self) -> None:
        # Tek parametre şablonu (concat tuzağı) — kazara değişime karşı sabitlenir.
        assert list(ENCODE_TEMPLATE) == [
            "-c:v", "libx264",
            "-preset", "medium",
            "-crf", "20",
            "-pix_fmt", "yuv420p",
    ...
    def test_iki_segmentte_girdi_disi_birebir_ayni(self, tmp_path: Path) -> None:
        ...
        c1 = build_segment_command(GIRDI, _keep(0, 2_000), tmp_path, 1)
        c2 = build_segment_command(GIRDI, _keep(3_456, 5_000), tmp_path, 2)
        ...
        assert encode_dilimi(c1) == ["-fps_mode:v", "cfr", *ENCODE_TEMPLATE]


class TestBuildSegmentCommand:
    def test_input_seeking_ss_iden_once(self, tmp_path: Path) -> None:
        cmd = build_segment_command(GIRDI, _keep(1_500, 4_000), tmp_path, 3)
...
class TestRenderMock:
    ...
    def test_mutlu_yol_segment_sayisi_concat_listesi_ve_temizlik(
        ...
        with (
            ...
            sonuc = render(girdi, _plan([(0, 2_000), (3_000, 5_000)]), cikti)
    ...
    def test_segment_hatasi_hangi_segment_ve_stderr_kuyrugu(
        ...
        with (
            ...
            render(girdi, _plan([(0, 2_000), (3_000, 5_000)]), tmp_path / "cikti.mp4")
    ...
    def test_zaman_asimi_render_error(self, girdi: Path, tmp_path: Path) -> None:
        with (
            ...
            render(girdi, _plan([(0, 1_000)], 1_000), tmp_path / "cikti.mp4", timeout=1)
...
class TestGercekFfmpegKesimDogrulugu:
    ...
    def test_cikti_suresi_plani_izler(self, tmp_path: Path) -> None:
        ...
        cikti = render(kaynak, plan, tmp_path / "cikti.mp4")
